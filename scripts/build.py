...
PACKAGE_LICENSE = "MIT"
PACKAGE_URL = "https://github.com/BKasin/gogios"
MAINTAINER = "bailey@gingertechnology.net"
VENDOR = "GingerTechnology"
DESCRIPTION = "Simple system to check important services are on remote machines."
...
def package_scripts(build_root, config_only=False):
    ...
    if config_only:
        ...
        shutil.copyfile(INIT_SCRIPT, os.path.join(
            build_root, SCRIPT_DIR[1:], INIT_SCRIPT.split('/')[1]))
        os.chmod(os.path.join(
            build_root, SCRIPT_DIR[1:], INIT_SCRIPT.split('/')[1]), 0o644)
        shutil.copyfile(SYSTEMD_SCRIPT, os.path.join(
...
def main(args):
    ...
    if args.release and args.nightly:
        logging.error("Cannot be a nigtly and a release.")
        return 1
